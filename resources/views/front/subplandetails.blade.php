...
<div class="container">
    <div class="row mx-auto">
        <div class="col-md-12">
            <p class="fw-bold mb-1">You will be Charged {{AppHelper::setCurrency($data->price)}}
                @if(auth()->user()->freetrial() && $data->free_trial>0 )
                 After Free Trial
                @elseif(!auth()->user()->freetrial() && $data->free_trial>0)
            ...
            <form action="javascript:;" method="post" id="activateplan">
                ...
                <button id="card-button" data-route="{{ route('front.subplan.submit',$data->id) }}" class="btn btn-primary btn-lg w-100" data-secret="{{ $intent->client_secret }}">
                     <i></i>
